...
export default function FixedPricesManagementPage() {
  ...
  const fetchFixedPrices = async () => {
    try {
      ...
      switch (currentTab) {
        case 1:
          response = await financialService.getArchivedFixedPrices();
          break;
        default:
          response = await financialService.getActiveFixedPrices();
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditMode && selectedPrice) {
        await financialService.updateFixedPrice(selectedPrice.id, payload);
        setSuccess('Fixed price updated successfully');
        setEditDialogOpen(false);
      } else {
        await financialService.createFixedPrice(payload);
  ...
  const handleDelete = async () => {
    ...
    try {
      setActionLoading(true);
      setError('');
      await financialService.deleteFixedPrice(selectedPrice.id);
  ...
  const handleRestore = async () => {
    ...
    try {
      setActionLoading(true);
      setError('');
      await financialService.restoreFixedPrice(selectedPrice.id);
  ...
  const handleForceDelete = async () => {
    ...
    try {
      setActionLoading(true);
      setError('');
      await financialService.forceDeleteFixedPrice(selectedPrice.id);

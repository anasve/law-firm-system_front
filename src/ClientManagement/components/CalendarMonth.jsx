...
export default function CalendarMonth({ lawyerId, selectedDate, onDateSelect }) {
  ...
  const fetchCalendarMonth = async () => {
    ...
    try {
      setLoading(true);
      setError('');
      const response = await appointmentsService.getCalendarMonth(lawyerId, year, month);
  ...
  const handleDateClick = (dateStr) => {
    ...
    // Check if date is marked as holiday in API
    const dayData = calendarData?.calendar?.[dateStr];
    if (dayData?.is_holiday) return;

    // Allow clicking on any future date (not past, not Friday, not holiday)
    // The system automatically generates slots from 8 AM to 2 PM
  ...
  const getDaysInMonth = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      // Even if API doesn't return available_count yet, slots will be generated automatically
  ...
  if (!lawyerId) {
    return (
      <Alert severity="info" sx={{ backgroundColor: colors.lightBlack, color: colors.white }}>
        Please select a lawyer first to view the calendar.
      </Alert>
    );
  }
        ...
        <>
          ...
          <DaysContainer>
            {days.map((dayInfo, index) => {
              ...
              const { day, dateStr, isHoliday, isPast, isToday, isSelected, hasSlots, availableCount, bookedCount } =
                dayInfo;

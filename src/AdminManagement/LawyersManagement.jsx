      ...
      export default function ManagementPage() {
        ...
        const handleArchiveLawyer = async (lawyer) => {
          console.log('Archiving lawyer:', lawyer);
          const token = localStorage.getItem("adminToken");
          if (!token) {
            navigate("/login");
            return;
          ...
          const apiData = new FormData();
          apiData.append("status", "archived");
          apiData.append("_method", "PUT");

          try {
            await axios.post(
              `http://127.0.0.1:8000/api/admin/lawyers/${lawyer.id}`,
              apiData,
              {
                headers: { Authorization: `Bearer ${token}` },
              }
            );
            // جلب القائمة من جديد بعد الأرشفة
            fetchLawyers(token);
          } catch (error) {
        ...
        const handleUnarchiveLawyer = async (lawyer) => {
          // This logic now lives in ArchivePage.jsx
        };
        ...
        const handleArchiveEmployee = async (employee) => {
          ...
          try {
            await axios.post(
              `http://127.0.0.1:8000/api/admin/employees/${employee.id}`,
              apiData,
              {
                headers: { Authorization: `Bearer ${token}` },
              }
            );
            // Remove from local state and navigate to archive page
            setEmployees(employees.filter(e => e.id !== employee.id));
          } catch (error) {
            console.error("Failed to archive employee:", error);
            alert("حدث خطأ أثناء أرشفة الموظف.");
          }
        };
                  ...
                  {filteredLawyers.length === 0 ? (
                    ...
                    <Grid key={lawyer.id} item xs={12} sm={6} md={4}>
                      <UserCard
                        ...
                        onDelete={() => handleDeleteUser(lawyer, "lawyer")}
                      />
                    </Grid>
                  ))}
                </Grid>
                  ...
                  {filteredEmployees.length === 0 ? (
                    ...
                    <Grid key={employee.id} item xs={12} sm={6} md={4}>
                      <UserCard
                        ...
                        onView={handleViewUser}
                        onDelete={() => handleDeleteUser(employee, "employee")}
                      />
                    </Grid>
                  ))}
                </Grid>
